var TimeKnots = {
  draw: function(id, events, options){
    ...
    if(cfg.showLabels != false){
      svg.selectAll("text")
        .data(events).enter().append("text").text(d => d.name).style("font-size", "70%")
        .attr("x", function(d){
          if(cfg.horizontalLayout){
            var datum = (cfg.dateDimension)?new Date(d.date).getTime():d.value;
                var x=  Math.floor(step*(datum - minValue) + margin/2);
                return x;        
          } 
          return Math.floor(cfg.width/2) //??
        })
        .attr("y", function(d){
           if(cfg.horizontalLayout){
             return Math.floor(cfg.height/2+(margin+this.getBBox().height)) - 55;
           }
           return margin+this.getBBox().height/2;
        });

        svg.selectAll("text2")
        .data(events).enter().append("text").text(d => d.date.substring(0, 10)).style("font-size", "70%")
        .attr("x", function(d){
          if(cfg.horizontalLayout){
            var datum = (cfg.dateDimension)?new Date(d.date).getTime():d.value;
                var x=  Math.floor(step*(datum - minValue) + margin/2);
        ...
        .attr("y", function(d){
           ...
           return margin+this.getBBox().height/2 - 100;
        });
    }
    ...
    //   }
      ...
      // svg.append("text")
      //    .text(events[0].name).style("font-size", "70%")
      //    .attr("x", function(d){if(cfg.horizontalLayout){return d3.max([0, (margin-this.getBBox().width/2)])} return Math.floor(this.getBBox().width/2)})
      //    .attr("y", function(d){
      //       if(cfg.horizontalLayout){
      //         return Math.floor(cfg.height/2+(margin+this.getBBox().height)) - 55;
